...
const greetUser = user => `Welcome back, ${user}.`  //arrow function online, template string

// console.log(greetUser('Carlo'))
...
//CODE HERE
//create function with for loop to loop through zip and look at index, if index does not equal allowed zip, console.log `Sorry, we can't deliver to that address`
...
// CODE HERE
let zipUnknown = []
const zipAnalyzer2 = zip => {
    zipUnknown.push(zip);

    for (let j = 0; j < zipUnknown.length; j++){;
    
    for(let i = 0; i < deliveryAreaZipCodes.length; i++)
        if (j[0] === i[0])
            console.log("You're in our delivery zone!")
     else {
        console.log("Sorry, we don't deliver to that address.")
        }
    }}

    // console.log(zipAnalyzer2(85004))
